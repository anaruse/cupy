...
from pynvrtc.interface import NVRTCException
...
def _get_nvrtc_version():
    ...
    if _nvrtc_version is None:
        _nvrtc_version = pynvrtc.__version__
...
def nvrtc(source, options=(), arch=None):
    ...
    with TemporaryDirectory() as root_dir:
        ...
        prog = Program(six.b(source), six.b(source))

        ptx = prog.compile(
            [six.b('-arch'), six.b(arch)] + [six.b(o) for o in options])

        with open('%s.cubin' % path, 'wb') as bin_file:
            bin_file.write(six.b(ptx))

        return six.b(ptx)


def preprocess(source, options=()):
    pp_src = nvrtc(source, options)
